...
const SidebarCourseItem = ({ course }: SidebarCourseItemProps) => {
  ...
  const normalizedDescription =
    course.description && course.description.toString().trim() !== ''
      ? course.description
      : '---';
    ...
    <HoverCard openDelay={700} closeDelay={100}>
      <HoverCardTrigger asChild>
        ...
        >
          ...
          <div
            className={cn(
              'grid grid-cols-[1fr_auto] gap-x-2 items-center flex-1 min-w-0',
              getFontSizeClass(),
            )}
          >
            {/* Left column: name + instructor (no description here) */}
      ...
      >
        <div className="space-y-2">
          ...
          {/* Description section - only in hover, always rendered */}
          <div className="mt-3 pt-3 border-t border-border/50">
            <span className="block mb-1 text-muted-foreground">
              {t('course.labels.description')}
            </span>
            <div className="text-foreground/90 text-[11px] max-h-20 overflow-y-auto leading-snug bg-muted/40 rounded-md px-2 py-1.5">
          ...
          <div className="pt-2 border-t border-border">
            <p className="text-[10px] text-muted-foreground mb-1">
              {t('course.labels.sessions')}
            </p>
            <div className="space-y-0.5">
              {course.sessions.map((session, idx) => (
                <div
                  key={idx}
                  className="text-[10px] text-foreground/80 flex items-center gap-2"
                >
